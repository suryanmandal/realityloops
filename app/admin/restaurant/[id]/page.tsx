...
export default function RestaurantDetail() {
  ...
  const handleToggle3D = async () => {
    ...
    try {
      const newStatus = !restaurant.is3dEnabled;
      const response = await fetch(`${process.env.NEXT_PUBLIC_API}/api/v1/admin/restaurant/account/${restaurantId}`, {
        ...
        body: JSON.stringify({ is3dEnabled: newStatus }),
      });

      if (response.ok) {
        setRestaurant(prev => prev ? { ...prev, is3dEnabled: newStatus } : null);
      } else {
        console.error('Failed to update 3D permission');
        alert('Failed to update 3D permission');
      }
    } catch (error) {
      console.error('Error toggling 3D permission:', error);
      alert('Error toggling 3D permission');
    }
  };
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center justify-between p-3 bg-blue-50 rounded-xl border border-blue-100 mt-4">
                      <div>
                        <p className="text-xs font-semibold text-blue-900">3D Model Access Permission</p>
                        <p className="text-[10px] text-blue-700">Allow restaurant to manage and view 3D AR models</p>
                      </div>
                      <label className="relative inline-flex items-center cursor-pointer">
                        <input
                          type="checkbox"
                          checked={restaurant.is3dEnabled || false}
